...
> ### ⚠ 2026-08-16 — 색 값의 정본은 이 문서가 아니다
>
> 약사로 브랜드가 여러 저장소(케어·밸런스·랜딩·터)로 뻗어나가면서, 같은 그린 `#0E6E54`가
> 저장소마다 다른 변수명으로 갈리는 문제가 생겼다. 그래서 **색 토큰 값만** 브랜드 공용 SSOT로 옮겼다.
>
> | 무엇 | 정본 |
> |---|---|
> | **색 토큰 값** | `C:\Users\main\yaksaro-hq\brand\tokens.json` 의 `brand.color` |
> | 화면 스펙 · 레이아웃 · 애니메이션 · 규제 문구 · 각 화면 수용기준 | **이 문서** (변함없음) |
>
> 아래 Colors 블록은 **참고용 스냅샷**이다. 값이 다르면 `tokens.json` 이 맞다.
> 실제로 이 문서는 이미 뒤처져 있다 — `blue500` 이 여기선 하나지만 코드에서는 장식용 `blue500` 과
> WCAG AA 를 만족하는 텍스트용 `blue700 #2E6CA6` 으로 갈라졌고, `saturday`·`infoText`·
> `warningTextSoft` 도 나중에 추가됐다.
>
> 색을 바꾸려면: `yaksaro-hq/brand/DECISIONS.md` 에 근거를 남기고 → `tokens.json` 을 고치고 →
> 생성기가 각 저장소에 반영한다. 이 문서나 `globals.css` 를 직접 고치지 않는다.
