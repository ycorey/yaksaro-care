...
> **2026-09-19 — 프로덕션 제출까지 끝났다.** 조직 계정("약사로") 개설 → 앱 생성 → 등록정보(원고·자산 17종)·앱 콘텐츠 선언 10종·
> Data safety 12유형 입력 → 내부 테스트 1(1.0.0) 게시 → 앱 서명 키 지문을 `assetlinks.json` 에 추가(PR #84·운영 반영) →
> 1차 제출(등록정보·선언 8건) **같은 날 승인·게시됨**(제출 활동: 15:57 제출 → "출시됨") →
> **프로덕션 1(1.0.0) 전체 출시 + 국가 대한민국 — 검토 제출(상태 "검토 중")**. 이 검토가 통과되면 스토어 일반 공개다.
> 프로덕션 심사는 보통 7일 이내(더 걸릴 수 있음). 상세는 `docs/play-submission.md` §4.
> 남은 것: ① 심사 결과 대기(통지: `ycorey@gmail.com` — admin@ 아님, 콘솔 이메일 알림 설정 실측) ② 통과 후 스토어 설치로 최종 확인
...
- [x] **`POST_NOTIFICATIONS` 실기기 수신 — 2026-09-19 확인.** 사용자가 서명 APK 를 실기기에 설치해 **알림 수신 확인**.
      ▸ 남은 꼬리: **알림 소리가 안 남** — 코드는 무음이 아니다(`public/sw.js` push 핸들러에 `silent` 없음 + `vibrate` 지정).
      Android 알림 채널 중요도 문제로 추정(TWA 위임 알림은 제조사에 따라 기본이 무음으로 잡힘, 특히 삼성).
      확인 경로: 알림 길게 누름 → 알림 설정 → 소리 허용 / 설정 → 앱 → 약사로케어 → 알림 → 채널 중요도 "기본" 이상 / 알림 볼륨·방해금지.
      기기 설정으로 해결 안 되면 그때 코드(채널 재생성 트리거 등)를 판다
