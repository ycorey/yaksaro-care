 ...
 *
 * ⚠️ 실측 결론(2026-06-22): I2570은 일반 가공식품(누룽지·두유·당절임)만 있고 건강기능식품이
 *   사실상 0건(루테인·밀크씨슬 검색 0). 게다가 우리 supplements는 품목제조신고명이라 리테일
 *   상품명과 이름 매칭도 안 됨 → 무료 데이터로 건기식 바코드 적재는 불가. 실행 보류.
 *   진짜 커버리지는 코리안넷(GS1 Korea) 유료 API = v2. 그전까지 건기식은 검색 폴백.
 * (참고) I2570 원본은 대한상공회의소 유통물류진흥원 제공, 2018 이후 갱신 중단.
 *
