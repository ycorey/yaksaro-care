...
전체 이력은 **[docs/HARNESS-CHANGELOG.md](docs/HARNESS-CHANGELOG.md)** 에 있다.
이 파일은 매 턴 컨텍스트에 실리므로 이력 본문은 여기에 쓰지 않는다 — 새 항목은 위 파일 맨 아래에 추가하고,
아래 "최근" 목록만 5건으로 유지한다.

**최근:**
- **2026-09-19** — **Play 프로덕션 제출** — 조직 계정 → 앱 생성 → 등록정보·선언 10종·Data safety → 내부 테스트 1(1.0.0) → `assetlinks` 병기(PR #84) → 1차 8건 **당일 승인** → 알림 실기기 수신 확인 → **프로덕션(대한민국) "검토 중"**. 통지: ycorey@gmail.com
- **2026-08-31** — Play 출시 선결 — `/interactions` 삭제·심사자 입구·§23 동의 기록(6/7이 미기록이었다)·고지 3종
- **2026-09-01** — `.aab` 서명·키스토어 백업 완료 · **상시 `rls-gate` 신설**(db-gate 는 조건절이 아니라 시크릿 부재였다) · 고지문의 없는 기능 삭제 · 틀린 서술 8건 정정
- **2026-09-04~05** — 건강 앱 = **조직 계정 + D-U-N-S 필수** 확정 · 스토어 스크린샷 5컷·특성그래픽 생성 파이프라인(`scripts/store-assets/`)
- **2026-09-18** — Play 제출 자료 완성(등록정보 원고·심사자 계정·Data safety 재정비) · 6렌즈 검수 51건 반영 — 원고의 "외부 전송 전 비식별화" 는 **거짓**이었다 · `assetlinks` 갱신은 출시 후가 아니라 **심사 전** · 처리방침에 없던 수집 4종 발견 → 개정안
