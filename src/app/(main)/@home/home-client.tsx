...
import { Wallet, Heart, Calendar, Send, Settings, type LucideIcon } from 'lucide-react'
...
const GRID_ITEMS: {
  ...
  { href: '/calendar', Icon: Calendar, iconColor: 'text-yc-warning',  iconBg: 'bg-yc-warningBg', title: '복약 캘린더',      desc: '날짜별 복용 기록을 봐요',      statKey: null },
  { href: '/share',    Icon: Send,     iconColor: 'text-yc-green600', iconBg: 'bg-yc-green50',   title: '의사·약사 보여주기', desc: '진료·조제 시 한 화면으로',     statKey: null },
]
  ...
  return (
    <div className="space-y-5 pb-4">
      ...
      <div>
        ...
        <h1 className="font-display text-[26px] text-yc-neutral900 mt-0.5">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          {GRID_ITEMS.map(({ href, Icon, iconColor, iconBg, title, desc, statKey, filled }) => (
            <Link key={href} href={href}
              className="bg-white rounded-yc-lg p-4 border border-yc-neutral200 shadow-[var(--yc-shadow-sm)] active:scale-95 transition-transform"
            >
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-3 ${iconBg}`}>
                <Icon size={22} className={iconColor} fill={filled ? 'currentColor' : 'none'} strokeWidth={filled ? 1.5 : 2} />
              </div>
              <p className="font-display text-[15px] text-yc-neutral900 leading-snug">{title}</p>
