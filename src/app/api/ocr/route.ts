...
export async function POST(request: Request) {
  ...
  try {
    ...
    // 파이프라인 하드 실패(외부 API 미도달 등)를 "약 0건"과 구분한다.
    // 전에는 여기서도 200 + 빈 목록으로 내려가 앱이 "다시 찍어보세요" 로 안내했다 —
    // 사진 문제가 아닌데 사진을 다시 찍게 만든다(2026-08-21 일시 장애에서 실제 발생.
    // 사용자는 재촬영·재시도를 반복하다 "OCR 이 아예 안 된다" 로 인식했다).
    // 503 이면 클라이언트가 이 문구를 그대로 보여주고 재시도 버튼을 유지한다.
    // 알려진 한계: consumeQuota 가 이미 차감된 뒤라 서버 실패에도 하루 한도가 준다
    // (환불은 감소 RPC 마이그레이션이 필요해 보류).
    return NextResponse.json(
      { error: '서버가 잠시 처방전을 읽지 못했어요. 사진은 그대로 두고 잠시 후 다시 시도해 주세요.' },
      { status: 503 },
    )
  }
