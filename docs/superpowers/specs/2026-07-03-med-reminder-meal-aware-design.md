...
### 선택(Optional) — 이번 범위에서 제외 (사용자 결정 2026-07-03)

- ~~설정 화면 "약이 있는 끼니에만 알림이 가요" 안내 문구~~ → **제외**
- ~~홈/오늘 인라인 슬롯 계산을 `effectiveMealSlots`로 교체(중복 제거)~~ → **제외**

이번 작업은 핵심 2곳(`effectiveMealSlots` 헬퍼 + 크론 필터)만 한다.
